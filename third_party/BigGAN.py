...
class G_D(nn.Module):
    ...
    def forward(self, z, gy, x=None, dy=None, train_G=False, return_G_z=False, policy=False):
        ...
        D_out, D_out_cls = self.D(D_input, D_class)

        if G_z is None:
            return D_out, D_out_cls, D_class
        elif x is not None:
            return torch.split(D_out, [G_z.shape[0], x.shape[0]]), torch.split(D_out_cls, [G_z.shape[0], x.shape[0]]), torch.split(D_class, [G_z.shape[0], x.shape[0]]), torch.split(D_input, [G_z.shape[0], x.shape[0]])
        else:
            if return_G_z:
                return D_out, D_out_cls, G_z, D_class
            else:
                return D_out, D_out_cls, D_class
